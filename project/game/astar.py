...
class AStarPlanner:
    ...
    # Create a node class, information including: xy coordinates, cost, parent node's index
    class Node:
        ...
        def __str__(self):  # return in type of string
            return str(self.x) + "," + str(self.y) + "," + str(
                self.cost) + "," + str(self.parent_index)

    # Input sx,sy,gx,gy, return pathx and pathy (the final path)
    def planning(self, sx, sy, gx, gy):
        """
        1. sx = nstart  sy = ngoal
        2. Initialize open_set & closed_set
        3. open_set = nstart
